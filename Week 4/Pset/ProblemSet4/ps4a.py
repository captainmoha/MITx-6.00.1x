...
WORDLIST_FILENAME = "C:/Users/Moha/Desktop/Spass/Python/6.00.1x Files/Week 4/Pset/ProblemSet4/words.txt"
...
def getWordScore(word, n):
    ...
    for letter in word:                               # iterate over the letters in word
        if letter in SCRABBLE_LETTER_VALUES:
            score += SCRABBLE_LETTER_VALUES[letter]

    score *= len(word)

    if len(word) == n: # if all the letters are used in the first time
        score += 50
    return score
...
def dealHand(n):
    ...
    for i in range(numVowels):
        x = VOWELS[random.randrange(0,len(VOWELS))]           # add random vowels to the hand
        hand[x] = hand.get(x, 0) + 1
        
    for i in range(numVowels, n):    
        x = CONSONANTS[random.randrange(0,len(CONSONANTS))]   # add random consonants to the hand
        hand[x] = hand.get(x, 0) + 1
...
def updateHand(hand, word):
    ...
    for letter in word:                                           
        if letter in hand:
            updatedHand[letter] = updatedHand.get(letter,0) - 1   # subtract 1 from a used letter
    print updatedHand
    return updatedHand
...
def isValidWord(word, hand, wordList):
    ...
    if (word in wordList) and satisfiesHand:     # conditions for a word to be valid
        return True
    else:
        return False
    ...
    while calculateHandlen(handNew) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                if calculateHandlen(handNew) == 0 or len(word) == n:
                    break
